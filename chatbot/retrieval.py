...
def retrieve_context(vector_store: VectorStore, question: str) -> list[Document]:
    try:
        ...
        # Store não suporta o filtro de metadados: cai para busca sem filtro.
        docs = _search(vector_store, question, use_filter=False)

    # Se o filtro removeu tudo (ex.: chunks antigos sem o campo "status" no
    # índice), refaz sem filtro para não perder resultados válidos.
    if not docs:
        docs = _search(vector_store, question, use_filter=False)
    ...
    if matchers:
        # Pergunta cita artigo/resolução/lei: trechos que contêm o termo vêm
        # primeiro; dentro de cada grupo mantém-se vigência e ano.
        docs.sort(
            key=lambda d: (
                0 if _doc_has_ref(d.page_content, matchers) else 1,
